...
const expectedFoundation = new Set([
  '@karaka/cordis',
  '@karaka/cosmokit',
  '@karaka/schemastery',
...
verifyPackages('packages/*/package.json', expectedApplications, 'application')

function verifyPackages(pattern, expected, kind) {
  for (const manifestPath of globSync(pattern)) {
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'))
    if (!expected.has(manifest.name)) throw new Error(`${manifestPath}: unexpected ${kind} package ${manifest.name}`)
  ...
  if (expected.size > 0) throw new Error(`missing ${kind} packages: ${[...expected].join(', ')}`)
}
