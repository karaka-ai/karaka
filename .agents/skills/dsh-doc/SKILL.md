...
- **Current state only.** Ordinary documentation describes the current codebase. Dedicated `persistence-change` records retain the before/after facts and compatibility evidence for their detected type transition; Agent Notes and postmortems retain their own historical scope.
