...
- **Current state only.** Ordinary documentation describes the current codebase. Dedicated `persistence-change` and `persistence-release` records retain type-transition evidence; release comparisons describe observed history without asserting compatibility. Agent Notes and postmortems retain their own historical scope.
