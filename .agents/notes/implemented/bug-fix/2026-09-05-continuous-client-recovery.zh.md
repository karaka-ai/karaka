...
长时间中断期间保留唯一重试调度，并以有界频率产生连接流量，直到恢复、显式停止或浏览器离线暂停。每次应用 WebSocket 尝试都通过配置的回调重新获取凭据。回调若持续返回无效凭据，仍需应用或用户处理；连接重试不会重放一元修改操作。立即就绪仍会重置退避，浏览器离线仍决定暂停；稳定连接后的重置窗口和本地载体例外属于独立策略变更。
