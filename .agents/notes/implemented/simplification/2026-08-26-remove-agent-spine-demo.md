...
Karaka retains the concurrent workspace and symlink-sensitive sandbox regressions under `sandbox-local`, using explicit testkit services and the real Agent loop. Each case owns temporary workspace directories beside the test because platform temporary areas are writable by sandbox policy; awaited plugin mounting replaces the demo's fixed readiness delay. Karaka's standalone launcher, embedded plugin exports, application ownership, and standing preset generations remain independent of the removed demo.
