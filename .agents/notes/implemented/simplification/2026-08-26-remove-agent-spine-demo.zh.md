...
Karaka 将并发工作区和符号链接敏感的沙箱回归测试保留在 `sandbox-local` 下，使用显式 testkit 服务与真实 Agent loop。每个用例在测试文件旁拥有临时工作区目录，因为平台临时区域按沙箱策略可写；等待插件挂载替代 demo 的固定就绪延迟。Karaka 的独立启动器、嵌入式插件导出、应用所有权和常驻 preset 代际均独立于被删除的 demo。
