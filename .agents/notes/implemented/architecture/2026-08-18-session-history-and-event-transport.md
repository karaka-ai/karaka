...
The Host sends one RFC 6455 Ping control frame to every open mux socket at the configured `websocketHeartbeatIntervalMs` interval (two seconds by default). The browser replies with Pong at the protocol layer; neither control frame enters the Remote stream JSON union or changes Connection generation state. The Host counts unanswered Ping intervals and resets the count on Pong. After two unanswered intervals, it defers its final termination check so a late Pong can preserve the socket.
