...
Karaka 使用 JSONL Session provider，保留持久化应用所有权、继承事件和请求去重。冷态 Agent 激活请求恢复其组合所需的 preset 投影；仅读取历史时省略投影计算。Karaka 录制场景使用同一 Session 根目录重启真实 Agent 进程，并检查冷态所有权验证、历史恢复及重复请求接收。provider 测试验证真实 fork 和重新挂载后保留所有权与继承事件。操作者已确认没有现有 chat 需要迁移：退役的 SQLite 数据库既不会被读取，也不会被删除，本次切换不提供转换器。参见[仅 JSONL 决策](../simplification/2026-08-30-jsonl-only-session-persistence.zh.md)。
...
Karaka 部署安装一个公开 runtime 标识，并可添加私有插件而无需发布它们。向内置 Loader registry 添加受支持的 DSH 模块时，也会添加其匹配的公开子路径；仅含约定的 entry 保持显式审计列表。内置构建必须保持动态模块加载、native asset、worker entry、declaration 和共享 Cordis instance 完整；clean packed-install test 负责此风险。内部 workspace 继续供 DSH 开发使用，但改变其模块图不要求对应的 Karaka 包图。JSONL 持久化 chat owner 和事件；待处理的结构化问题仍保留在进程内。TLS、副本数、跨副本路由和 supervision 仍由部署负责。

每个 Karaka home 和 Session 根目录只有一个活跃写入进程。JSONL 追加没有跨进程隔离，因此副本需要独立根目录和 Session 路由。首次物化的独占发布不构成跨进程追加锁。
