...
Karaka publishes three packages. `@karaka-ai/sdk` is the backend client and application MCP tool host. `@karaka-ai/agent` is the complete persistent Agent server. `@karaka-ai/cli` creates a project and launches the same-version Agent executable. Karaka publishes no separate harness, server, authentication, transport, or MCP bridge package.

The repository retains focused internal workspaces for development and tests. The `@karaka-ai/agent` build statically includes the production DSH and Karaka modules it uses; those workspace packages are build-time inputs, not consumer runtime dependencies. Ordinary third-party libraries remain package dependencies when embedding them would break native installation, singleton identity, or upstream asset loading.

Agent owns the process entry, base Cordis composition, Karaka overlay, runtime assets, and an embedded plugin registry. Public configuration names use `@karaka-ai/agent/*`. The Loader resolves those exact names from the embedded registry, resolves relative names from the file that declares them, and resolves other bare names from the Agent project. An application-specific plugin can therefore live under the server repository and load through `./` or `../`; a reusable plugin may remain an ordinary installed dependency.

Every embedded plugin name is also a public Node subpath with its original named and default exports. Contract-only Service Definition modules receive the same flat DSH-derived names without becoming Loader plugins. The Agent build emits the Loader and public entries through one shared runtime graph; public declaration facades share a private declaration tree with relative cross-package references, load their package-owned declaration augmentations, and contain no DSH package names. The build locates workspace declaration entries from package manifests, so a clean checkout does not require pre-existing workspace JavaScript bundles before public entries are generated. Agent projects depend directly on `@karaka-ai/agent`, so local `plugins/*.js` files and reusable packages compile against the same service identities that the Loader mounts.
...
Karaka uses the JSONL Session provider with durable application ownership, inherited events, and request deduplication. Cold Agent activation requests the preset projections required to restore its composition; history-only reads omit projection work. The recorded Karaka scenario restarts the real Agent process against the same Session root and checks cold ownership enforcement, history recovery, and duplicate request admission. Provider tests preserve ownership and inherited events across a real fork and remount. The operator confirmed that no existing chats require migration: the retired SQLite database is neither read nor deleted, and this cutover provides no converter. See the [JSONL-only decision](../simplification/2026-08-30-jsonl-only-session-persistence.md).
...
**Keep `@karaka-ai/harness` as a thin DSH profile.** This minimizes build work but requires the DSH launcher and all modified plugin packages at deployment time, which leaves Karaka as configuration over another installed product.

**Put the complete runtime in `@karaka-ai/cli`.** This makes a command package own the server implementation and prevents Agent extensions from depending on a stable runtime package without depending on CLI concerns.
...
A Karaka deployment installs one public runtime identity and can add private plugins without publishing them. Adding a supported DSH module to the embedded Loader registry also adds its matching public subpath; contract-only entries stay an explicit audited list. The embedded build must keep dynamic module loading, native assets, worker entries, declarations, and the shared Cordis instance intact; clean packed-install tests own that risk. The internal workspaces remain available to DSH development, but changing their graph does not require a corresponding Karaka package graph. JSONL persists chat ownership and events; pending structured questions remain process-local. TLS, replica count, cross-replica routing, and supervision remain deployment responsibilities.

Each Karaka home and Session root has one active writer process. JSONL appends are not fenced across processes, so replicas require separate roots and Session routing. Exclusive first materialization does not establish a cross-process append lock.
