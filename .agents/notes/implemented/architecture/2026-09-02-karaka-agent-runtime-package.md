...
Every embedded plugin name is also a public Node subpath with its original named and default exports. Contract-only Service Definition modules receive the same flat DSH-derived names without becoming Loader plugins. The Agent build emits the Loader and public entries through one shared runtime graph; public declaration facades share a private declaration tree with relative cross-package references and no DSH package names. Agent projects depend directly on `@karaka/agent`, so local `plugins/*.js` files and reusable packages compile against the same service identities that the Loader mounts.
