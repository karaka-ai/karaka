...
A Karaka deployment installs one public runtime identity and can add private plugins without publishing them. The embedded build must keep dynamic module loading, native assets, worker entries, declarations, and the shared Cordis instance intact; clean packed-install tests own that risk. The internal workspaces remain available to DSH development, but changing their graph does not require a corresponding Karaka package graph. SQLite persists chat ownership and events; pending structured questions remain process-local. TLS, replica count, cross-replica routing, and supervision remain deployment responsibilities.
