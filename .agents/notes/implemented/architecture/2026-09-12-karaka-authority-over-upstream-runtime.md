...
Karaka HTTP operations call the original engine through its public APIs. The profile uses the upstream DSH launcher and Loader readiness signal. Application routes reject requests until the entire plugin tree is ready. The separately maintained SDK consumes those routes without embedding another Session implementation.
...
**Use stock MCP unchanged for application tools.** Its public API lacks fetch-time credentials, trusted execution metadata and an unpublished application catalog. A bounded adaptation retains existing reconnect and result-conversion behavior without modifying upstream MCP.

## Consequences

Upstream engine updates can retain original files. Karaka maintains its authority and application adapters, plus two explicitly derived MCP files. Ownership and Session files form one application backup; a Session JSONL file alone does not establish application ownership. Publication spans two files rather than a cross-file transaction, so ordering and recovery are part of the authority contract.
...
Evidence is recorded in `/tmp/karaka-application-flow-jZS1P4/evidence.json` and retained by the updater at `work/reuse-implementation/evidence.json`. Dependency installation, native-addon build, runtime/browser bundles and public declaration emission prepare execution. No other tests or suites ran. Release artifacts are staged; registry publication is tracked separately. Broader recovery, concurrency, child/worker and browser behavior remain unverified by this scenario.
