...
The shared public MCP factory now receives `call(args, execution)`. Metadata resolves directly from that exact execution before the SDK request, preserving its abort signal and tool definition. The earlier execution-local delegate is no longer needed: upstream's factory owns output schemas, canonical results, image projection and finalization directly. Karaka retains only metadata, preparation, scoped registration and transport extensions. Ordinary MCP configuration publishes server resources and instructions through its own plugin lifecycle; Karaka's direct connection entry does not publish application resources or instructions globally. The [upstream source record](../../../../packages/karaka/mcp-application/UPSTREAM.json) retains the original shared-adoption revision.
