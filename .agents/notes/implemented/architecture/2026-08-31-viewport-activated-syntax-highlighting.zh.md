...
普通 jsdom 组件测试通过仅用于测试的 observer 将已挂载根元素建模为可见；生产代码直接使用浏览器 API。可控视口测试覆盖激活前源码增长、语言变化、卸载与过期通知。完整浏览器测试通过真实 observer 驱动屏幕外代码围栏与读取卡片，验证激活时的几何尺寸、复制和选择，并确认滚出视口后保留高亮 DOM。
