...
`useViewportHighlighting` 为可进行语法高亮的 `CodeBlock` 和 `ReadBlock` 实例持有一个共享的 `IntersectionObserver`。受支持的块在根元素首次与视口相交之前渲染既有的纯文本臂。语言缺失或不受支持时不会向 observer 注册。
...
普通 jsdom 组件测试与基于构建 bundle 的组装测试共用仅用于测试的 observer，将已挂载根元素建模为可见；生产代码直接使用浏览器 API。Web 测试配置在组装启动前安装此模型，构建启动冒烟测试要求读取卡片渲染高亮 token。可控视口测试覆盖激活前源码增长、语言变化、卸载与过期通知。完整浏览器测试通过真实 observer 驱动屏幕外代码围栏与读取卡片，验证激活时的几何尺寸、复制和选择，并确认滚出视口后保留高亮 DOM。
