...
共享的公开 MCP 工厂来自[上游 PR #3304](https://github.com/deepseek-ai/deepseek-harness/pull/3304)；来源记录仍保留最初采用共享实现时的精确修订号。该工厂保留双参数 `call(args, signal)` API。启用元数据的发现流程将每次执行委托给该工厂，捕获可信调用方，而不使用全局调用方变量或信号到所有者的映射。以执行对象为键的 WeakMap 将匹配的委托保留到终结阶段，使原有模式、规范结果及图像投影规则继续生效。拒绝时移除该次调用的委托；终结时移除保留的委托，并原样转交策略处理后的最终结果。此清理释放投影引用，不回滚工厂已经保存的持久附件字节。
