...
Karaka 负责经过认证的 HTTP 传输、私有目录注册、继承策略检查及小型模式验证包装器。包装器在准备阶段验证受支持的对象模式，在受支持调用进入共享执行器前验证参数，保留本地 `INVALID_ARGS` 分类。要求任务扩展的工具保留共享实现中不支持任务的拒绝行为。连接监督、协议命名、丰富结果转换及其继承测试在 DSH 中只有一个维护者。[上游来源记录](../../../../packages/karaka/mcp-application/UPSTREAM.json)列出承载扩展的三个共享实现文件和本地适配器。
