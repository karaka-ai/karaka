...
[标准托管运行器拓扑](2026-09-02-standard-hosted-pull-request-runners.zh.md)让静态门禁清单和构建后消费方清单分别运行在独立作业中。在此前的拓扑中，二者共用的构建由静态作业负责：静态作业要等所有静态门禁完成后才上传生成的目录树，消费方作业则在恢复该目录树前声明作业级依赖。基于编译输出的快照与发布校验确实需要完整构建，但不依赖运行时依赖闭包检查、文档生成或模块图验证。
