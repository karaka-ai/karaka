...
The [standard hosted runner topology](2026-09-02-standard-hosted-pull-request-runners.md) gives the static and built-consumer inventories separate jobs. In the earlier topology, the static job owned their shared build: it uploaded the emitted tree only after every static gate completed, and the consumer job declared a job-level dependency before restoring that tree. Compiled-output snapshots and publication checks genuinely require a complete build; they do not require runtime-closure checks, documentation generation, or module-graph verification.
