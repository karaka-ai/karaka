...
`DSH_COVERAGE_MAX_WORKERS` continues to size the uninstrumented exempt gate and the ordinary non-partitioned path; it does not resize partition children. Pull-request coverage gives the exempt gate one worker and admits two concurrent outer gates. Both PR coverage jobs run without a preceding build. Workspace imports resolve to `src`, while the packer image, runtime corpus, and trajectory bundle suites skip their artifact assertions on an unbuilt checkout. The serial Windows complete reference still waits for its build before coverage, so those assertions consume complete artifacts there. Linux and Windows overlap two instrumented partition processes with one exempt worker while keeping every instrumented process single-worker.
...
`scripts/coverage-partitions.spec.ts` pins argument construction, package-script separator removal, one-worker partitions, weighted longest-processing-time assignment (including a case that fails when assignment ignores recorded weights), the single merged threshold command, failed-test merging, failure diagnostics before complete-blob validation, waiting for sibling partitions after a spawn failure, and link-safe cleanup. `scripts/run-gates.spec.ts` pins opt-in selection, invalid-count rejection, both coverage gates' build dependency in the complete Windows reference, the complete Windows inventory with its blocking split, and unbuffered streamed output. React fake-timer cases that can move between partitions advance timers inside `act()`; geometry-dependent portal tests stub their element rectangles so a different shard schedule cannot turn deferred updates or jsdom coordinates into coverage-only failures.

Completed comparisons measured two Linux partitions at 276.68 and 282.27 seconds and two native Windows partitions near 405 seconds under an earlier gate ordering. Automatic pull-request coverage uses that stable two-partition configuration on standard hosted runners: two single-worker instrumented processes run beside one exempt-heavy worker, with two top-level gates active. The same budget applies under self-hosted failover so switching pools does not change the test schedule. Historical sixteen-partition Windows samples reached 112.66–122.01 seconds, but that latency profile requires larger capacity and is retained only as comparison evidence. These values come from completed runs or fixed capacity bounds; an unfinished run crossing an arbitrary elapsed-time mark is not evidence for increasing concurrency.
