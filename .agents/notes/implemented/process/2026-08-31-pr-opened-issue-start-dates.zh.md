...
启用 `initializeStartDates` 时，Issue 生命周期工作流仅在 `pull_request.opened` 时初始化 `Start Date`。工作流读取 PR 的实时正文，保留每个能解析为 Issue 的同仓库引用，把 `created_at` 按配置的 Project 时区转换为日历日期，确保 Issue 是 Project item，并仅在当前值为空时写入配置的 Date 字段。

Karaka 保持 `initializeStartDates: false`，直到经过授权的读取验证配置的 Project 字段和 CI 凭据。禁用初始化时不读取或写入日期，也不要求日期配置。配置的字段名为 `Start Date`，时区为 `Asia/Kolkata`；这两个配置项都不能证明实际 Project 访问权限。启用初始化时，日期写入前会拒绝缺失配置、无效时区或时间戳、缺失字段、非 Date 字段和 Issue 级字段。API 失败会明确报错。
...
[Issue 管理测试](../../../../.github/issue-management/policy.test.mjs)覆盖加尔各答时区日期边界、仅 opened 分派、同仓库 Issue 过滤、空值写入、已有值保留、Project item 缺失、无效字段和 Issue 级字段，以及 GraphQL mutation 变量。测试还证明禁用初始化时不会发出请求，缺少日期配置时普通 Issue 状态更新仍可运行。使用 mock 的 API 测试不能验证实际 Project schema 或凭据访问权限。[工作流测试](../../../../scripts/ci-workflow.spec.ts)要求保留 `pull_request.opened` 订阅。
...
只有启用初始化期间处理的 PR opened 事件会初始化日期。创建后新增的引用和现有开放 PR 保持不变，工作流不会扫描已有 Project item 或 PR。
