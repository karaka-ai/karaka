...
The process-table walk visits each PID once and excludes the root from its descendants, even when rows repeat or parent relationships form a cycle. Its traversal queue is separate from the observed child lists, and child enumeration does not expand those lists into function arguments. This bounds work by the observed table instead of trusting it to be an acyclic tree.
