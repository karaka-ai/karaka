...
When `initializeStartDates` is enabled, the Issue lifecycle workflow initializes `Start Date` only for `pull_request.opened`. It reads the pull request's live body, retains every same-repository reference that resolves to an Issue, converts `created_at` to a calendar date in the configured Project time zone, ensures the Issue is a Project item, and writes the configured Date field only when the current value is empty.

Karaka keeps `initializeStartDates: false` until an authorized read verifies the configured Project field and CI credentials. Disabled initialization performs no date reads or writes and does not require date configuration. The configured field name is `Start Date` and the time zone is `Asia/Kolkata`; neither config entry proves live Project access. Enabled initialization rejects missing configuration, invalid time zones or timestamps, missing fields, non-Date fields, and Issue-level fields before writing a date. API failures remain visible.
...
[Issue-management tests](../../../../.github/issue-management/policy.test.mjs) cover the Kolkata date boundary, opened-only dispatch, same-repository Issue filtering, empty-value writes, existing-value preservation, missing Project items, invalid and Issue-level fields, and GraphQL mutation variables. They also prove disabled initialization makes no requests and leaves ordinary Issue status updates operational without date configuration. Mocked API tests do not verify live Project schema or credential access. [Workflow tests](../../../../scripts/ci-workflow.spec.ts) require the `pull_request.opened` subscription.
...
Only pull-request opened events processed while initialization is enabled initialize dates. References added after creation and existing open pull requests remain unchanged, and the workflow does not scan existing Project items or pull requests.
