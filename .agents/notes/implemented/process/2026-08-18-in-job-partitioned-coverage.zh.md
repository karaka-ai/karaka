...
`DSH_COVERAGE_MAX_WORKERS` 继续控制无插桩豁免门禁和普通非分区路径的规模，不会调整分区子进程。拉取请求覆盖率为豁免门禁分配 1 个 worker，并允许 2 道外层门禁并发。Windows 工作区构建会在两道覆盖率门禁启动前完成。插桩套件包含针对已构建 `lib/` 输出的打包器断言，因此这项依赖可避免它读取只完成部分产出的包闭包，也可避免豁免门禁的临时 Oxlint 探针与源码编译竞态。Linux 与 Windows 都让 2 个插桩分区进程与 1 个豁免 worker 重叠运行，同时保持每个插桩进程只有 1 个 worker。
