...
普通的 `pnpm run test:coverage` 命令仍只启动一次 Vitest。Linux 与原生 Windows 拉取请求覆盖率会为标准托管容量把 `DSH_COVERAGE_PARTITIONS` 固定为 2。运行期间不会由任何耗时触发器改变这两个数量。[覆盖率豁免重型套件](2026-07-31-coverage-exempt-heavy-suites.zh.md)仍作为独立的无插桩门禁与插桩工作并排运行。
...
`DSH_COVERAGE_MAX_WORKERS` 继续控制无插桩豁免门禁和普通非分区路径的规模，不会调整分区子进程。拉取请求覆盖率为豁免门禁分配 1 个 worker，并允许 2 道外层门禁并发。两个 PR 覆盖率作业都不预先构建。工作区 import 解析到 `src`，而 packer image、runtime corpus 和 trajectory bundle 套件在未构建的 checkout 上跳过产物断言。串行 Windows 完整参考流程仍先等待构建再运行覆盖率，因此这些断言在那里消费完整产物。Linux 与 Windows 都让 2 个插桩分区进程与 1 个豁免 worker 重叠运行，同时保持每个插桩进程只有 1 个 worker。
...
`scripts/coverage-partitions.spec.ts` 固定了参数构造、包脚本分隔符移除、单 worker 分区、加权最长处理时间分配（含一个在分配忽略记录权重时必然失败的用例）、唯一一次合并阈值命令、失败测试合并、完整 blob 校验前的失败诊断、spawn 失败后等待兄弟分区，以及链接安全清理。`scripts/run-gates.spec.ts` 固定了显式启用、非法数量拒绝、Windows 完整参考流程中两道覆盖率门禁对构建的依赖、完整 Windows 清单及其阻断性划分，以及不缓冲的流式输出。可能在分区间移动的 React fake-timer 用例会在 `act()` 内推进计时器；依赖几何位置的 portal 测试会固定元素矩形，使不同分片调度不会把延迟更新或 jsdom 坐标变成只在覆盖率运行中出现的失败。

已完成的对比中，两个 Linux 分区耗时 276.68 秒和 282.27 秒，两个原生 Windows 分区在先前门禁顺序下耗时约 405 秒。自动拉取请求覆盖率会在标准托管运行器上使用这套稳定的双分区配置：2 个单 worker 插桩进程与 1 个豁免重型 worker 并行运行，同时活跃的顶层门禁为 2 个。自托管故障切换使用相同预算，因此切换池不会改变测试调度。历史 Windows 16 分区样本耗时 112.66–122.01 秒，但该延迟配置需要更大容量，现在仅作为对比证据保留。这些数值来自完整运行或固定容量上限；运行尚未结束时跨过任意耗时刻度，不构成增加并发的证据。
...
Linux 与 Windows 使用相同的协调器，并各自设置分区数量与外围 worker 预算。专用 Linux 与 Windows PR 覆盖率作业使用未构建的 checkout；Windows 完整参考流程在构建之后运行覆盖率。覆盖率门禁不与并发构建共享工作区。本地覆盖率默认保持简单；只有调用方显式选择分区包脚本并提供大于 1 的合法数量时，才启用分区。
