...
The active serial references run on the self-hosted `vm-backup` (`serial / linux`) and `dsh-win-ci` (`serial / windows`) pools; the one remaining disabled hosted serial reference (`serial-macos`) uses `macos-latest`, and there is no standard-hosted `serial / linux` label. The required pull-request Windows job runs under Wine on `ubuntu-latest`, while the independent pull-request native jobs use standard `windows-2025` under normal operation and the self-hosted `[self-hosted, dsh-win-ci, windows]` pool for trusted same-repository failover (see the [failover runbook](2026-07-26-ci-failover-runbook.md)). Required pull-request jobs use portable standard capacity under the [standard hosted runner decision](2026-09-02-standard-hosted-pull-request-runners.md). Higher-core hosted runners remain manual benchmarks because a correctness path must remain runnable without repository-external runner configuration.
