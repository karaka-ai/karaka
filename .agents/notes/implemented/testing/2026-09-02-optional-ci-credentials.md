...
The issue lifecycle workflow detects whether both GitHub App credential values exist before creating an installation token or mutating the configured project. Without them, it reports a notice and succeeds without project mutation. The Cloudflare preview workflow likewise reports a notice and skips checkout, build, deployment, verification, and commenting unless all four deployment and Access credentials exist. The separate issue-policy workflow remains a read-only required check using the workflow token. It checks out policy code from the default branch, then sets only the organization and repository fields from trusted event metadata before validation; other policy settings remain owned by the default branch.
...
A repository with no DeepSeek key, issue GitHub App, or Cloudflare credentials can run all required clean-build and deterministic checks without credential failures. Its CI does not prove live-provider compatibility, update its issue project, or publish a preview until the corresponding optional credentials are configured. The workflow UI exposes those omissions as skipped credentialed work or an explicit notice instead of treating them as source failures.
