...
The issue lifecycle workflow detects whether both GitHub App credential values exist before creating an installation token or mutating the configured project. Without them, it reports a notice and succeeds without project mutation. The separate issue-policy workflow remains a read-only required check using the workflow token, and repository policy configuration names the current `karaka-ai/karaka` repository.
