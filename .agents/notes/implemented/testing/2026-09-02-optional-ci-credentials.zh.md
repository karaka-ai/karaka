...
没有 DeepSeek 密钥和 issue GitHub App 的仓库仍能运行全部必需的干净构建与确定性检查，而不会出现凭据失败。其 CI 不会证明 live 提供方兼容性，issue project 也要等到配置这些可选凭据后才会更新。工作流 UI 会通过跳过带凭据的工作或显式 notice 展示这些缺失，而不是把它们当作源码失败。
