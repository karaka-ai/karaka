...
describe('headless stream-json snapshots', () => {
  ...
  it('keeps provider comments alive and sends DeepSeek defaults through the one-shot app', async () => {
    // Keep comment-only output longer than the 2s idle deadline, with room for CI scheduling.
    const server = await deepseekDefaultsServer({ keepAliveCount: 40, keepAliveIntervalMs: 100 })
    try {
      const result = await runLoaderSmoke({
