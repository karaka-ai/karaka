...
Every supported Node application runs through the `dsh` profile launcher. The shipped applications are `dsh web` (the deliberate alias for `--profile web`), `dsh --profile headless`, `dsh --profile sdk`, `dsh --profile sdk-minimal`, and `dsh --profile acp`. The TypeScript SDK resolves its same-version `dsh` dependency and selects `sdk`; custom plugin composition remains a profile plus ordered patch files, not an inline application tree. A product CLI may prepare local profile state and select one fixed profile before delegating to `dsh`; `karaka start` is this restricted wrapper for the `karaka` profile. `sdk-minimal` is a repository-owned standalone bundle behind the same launcher, not a caller-supplied Cordis tree.
...
Karaka is a persistent DSH profile for application-owned agents. The application backend installs [`@karaka/sdk`](../packages/karaka/sdk/README.md); the separate Karaka process installs [`@karaka/cli`](../packages/karaka/cli/README.md), the agent definitions, and any custom Cordis plugins those definitions reference. Installing the SDK does not start either process or open a port.

An agent definition is one DSH Agent Preset directory. `preset.yml` supplies discovery metadata such as name, description, and order. `agent.cordis.yml` is the behavioral plugin composition: persona, selected remote tools, skills, subagents, and other scoped contributions. Karaka mounts one standing Cordis plugin tree for each detected composition generation. Creating or resuming a chat gives its Agent scope the scoped contributions registered by its generation. Chats joined to the same generation share preset plugin instances but retain distinct Agent and Session state; a changed composition starts a new generation without disrupting chats still joined to the old one. A new chat starts with the deployment's default model selection; `chats.setModel()` takes effect on that chat's next request and remains its selection without changing the deployment default or other chats.
...
`karaka start` reuses the DSH launcher, Loader, Agent Presets, ReactLoopAgent, model adapters, tool registry, Session log, and SQLite persistence. The Karaka bundle disables local filesystem and subprocess tools by default. One launch owns one Node process; replica count, routing between replicas, and TLS termination remain deployment responsibilities.
