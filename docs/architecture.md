...
Karaka is a configurable, Cordis-based foundation for composing agentic SaaS runtimes. Stable capability seams define what the runtime can do, provider plugins decide how and where infrastructure work is done, and application configuration selects the product that runs. A backend-mounted MCP server plugin can turn decorated methods on framework-managed services into agent tools without requiring developers to author one plugin per method.

The repository publishes nine packages that form the composition kernel. Seam contracts, providers, and advanced extensions live in separately installable plugins built on that kernel. Authentication, an overall-spend Entitlement seam, provider-neutral Storage with a persistent local provider, an Agent Runtime with effect-owned agent plugins, durable sessions, text streaming and the validated tool loop, an OpenAI Responses model provider, the Tool authoring API, effect-owned core registry, application MCP server, and static-endpoint MCP client plugins, setup-YAML process bootstrap, authenticated HTTP and IPC Transports, and a matching application SDK exist today. Framework-specific MCP server adapters, dynamic endpoint discovery, richer chat operations, agent routing, skills, and subagent coordination described below are target architecture unless stated otherwise.
...
flowchart TB
  ...
  Discovery["Trusted MCP-endpoint discovery"]
  ServiceGraphs["Independent backend<br/>MCP server graphs"]
...
Loader imports the agent module exactly like every other configured plugin. Agent Runtime validates and indexes its effect-owned descriptor but does not interpret an agent-specific YAML language or create a parallel lifecycle. Registration compiles the referenced model and exact tool allowlist before the agent becomes active. Because Loader may start sibling entries concurrently, a missing contribution leaves the descriptor pending until the settled-graph readiness phase; unresolved dependencies, incompatible schemas, or a model without tool support then fail process readiness. Prompt, model, session, tool, skill, delegation, and child-plugin contributions therefore share normal Cordis dependency ordering, isolation, disposal, and replacement. Removing or reloading the agent plugin reverses its registrations. Direct programmatic Cordis composition remains available for Karaka internals, tests, embedded integrations, and advanced plugin authors, but setup YAML is the normal deployment surface.
...
A **tool** is an operation intentionally exposed to a model inside the Agent Runtime seam. The core Tool plugin provides `ctx.tools`, an effect-owned registry for model-visible names and JSON schemas. It compiles boundary validators once per registration, binds an explicit allowlist, validates input and output, drains active calls during disposal, and removes the contribution. The current application MCP server plugin binds decorated services to this registry and exposes them through MCP; the current MCP client plugin validates configured remote catalogs and contributes invocation clients to the same registry. Framework adapters, dynamic endpoint-discovery providers, and tool-policy implementations remain ordinary planned Cordis plugins in the Tool plugin family. MCP supplies the remote tool protocol; Karaka's application-facing Transport seam remains a separate Chat API boundary.
...
The `@karaka/sdk` package exports the metadata-only `tool` decorator, immutable JSON Schema 2020-12 descriptors, and invocation context types. These exports are inert: importing the SDK neither mounts behavior nor registers a tool. The `@karaka/tool/core` subpath exports the effect-owned registry plugin, `@karaka/tool/mcp-server` exports the application-side MCP server plugin, and `@karaka/tool/mcp-client` exports the static-endpoint Karaka-side client plugin. Future first-party runtime behavior will be exposed through plugin subpaths of the Tool package. Target naming includes framework-specific `@karaka/tool/mcp-server-*`, `@karaka/tool/discovery-*`, and `@karaka/tool/policy-*`. Each behavioral subpath exports an ordinary Cordis plugin, and every registration it makes is owned by a reversible effect. The exact list grows with implementations, but first-party Tool behavior must not escape this plugin contract.
...
`@tool` attaches immutable metadata to the method only. It is an inert authoring helper, not a second plugin system. Importing a decorated class does not register it or mutate a global registry. The application MCP server plugin receives backend-managed instances during application bootstrap, reads their tool metadata, binds the methods, and exposes them through one MCP endpoint using reversible Cordis effects. Its mount callback connects the web-standard endpoint to the backend's existing server and must return an unmount function. Future framework-specific adapters will supply that wiring automatically. The server plugin owns the handler, registrations, active-call cancellation, and unmount lifecycle. The decorator does not create another service container, lifecycle, registry, or non-disposable global side channel.
...
`@karaka/model-openai` is the first real model-provider plugin. It maps Agent Runtime conversation history and logical tool descriptors to the OpenAI Responses API, maps function calls and results back to provider-neutral items, supports completed and streamed text generations, forwards cancellation, and reports configured metered spend from returned token usage for completed responses. Setup selects the model, credentials, OpenAI `serviceTier`, and integer per-million-token rates. `serviceTier` is sent on every request and defaults predictably to `default`, not project-selected `auto`.
...
References contributed by an agent plugin name logical capabilities or policies, not concrete provider objects or endpoints. Verified MCP tool lists provide logical application-tool names; setup-selected model and session providers satisfy the other references. Replacing OpenAI with DeepSeek, PostgreSQL with another session backend, or one MCP endpoint-discovery provider with another therefore does not require changing the agent plugin unless its logical policy changes.
...
Activation performs structural validation once: it resolves the model, binds the exact allowed tool descriptors, and asks the model provider to validate that tool set. Model and tool registries expose monotonic revisions, so Agent Runtime recompiles only after one of those dependencies changes. Starting a turn then leases one immutable agent, model, and tool snapshot. Removing a plugin prevents new turns immediately while disposal waits for turns already holding that snapshot. Model-generated arguments and tool results remain runtime data and are validated on every invocation.
...
On every turn, including after a process restart, Agent Runtime resolves the stored agent ID against the current active compiled agent and loads durable chat state into that behavior. The turn leases that immutable snapshot through every model generation and tool call. Replacing or reloading an agent, model, or tool plugin therefore changes subsequent turns while an already-running turn completes against its leased implementations. The versioned session format preserves provider-neutral messages, tool calls and results plus JSON-safe provider-owned replay items. Provider plugins replay their own opaque items, ignore foreign provider-only items, and use the neutral form of messages and calls produced by another provider. They can therefore retain required stateless context such as OpenAI reasoning items and assistant phases without making Agent Runtime interpret it or preventing model-provider replacement. Exact historical execution is a separate deployment policy that requires retaining an old executable artifact; a chat record cannot preserve code that is no longer deployed.
...
`karaka start --config karaka.yaml` starts one persistent Karaka process from a top-level Loader entry list. The thin `@karaka/cli` process host creates the root Cordis context, mounts Loader and Include from the composition kernel, waits for the configured plugin graph to settle, dispatches the plugin-owned `karaka/ready` validation phase, and disposes the graph on `SIGINT` or `SIGTERM`. A failed readiness listener aborts startup and disposes the partial graph. The CLI does not own application behavior: setup-selected plugins provide the Chat API, transports, seams, providers, and agents.
...
flowchart LR
  ...
  Karaka -->|"authenticated MCP tools/call"| Invoice["Invoice-service MCP endpoint"]
  Karaka -->|"authenticated MCP tools/call"| Customer["Customer-service MCP endpoint"]
  Karaka --> Support["support agent plugin"]
  Karaka --> Billing["billing agent plugin"]
  Karaka --> Research["research agent plugin"]
```

The baseline production topology therefore has existing application services and one Karaka server. Every application deployment contains its decorated methods and a small MCP server plugin; the Karaka deployment contains an application-facing Transport plugin, Agent Runtime, agent plugins, model provider plugins, and Tool endpoint-discovery and MCP client plugins. Neither deployment absorbs the other's implementation. Karaka-native control capabilities may run locally as first-party plugins, but application business operations remain in the services that own their data and authorization.

For a microservice product, a small Karaka deployment project is the recommended assembly point. It contains the setup YAML, agent plugin modules, prompts, package manifest and lockfile, and optional local agent-side plugin source. Agent-side plugins owned by other repositories are published or otherwise installed into this deployment artifact. Microservice repositories keep their backend code, decorators, and MCP server plugin. The repository name and directory layout are conventions, not runtime contracts; a smaller or embedded product may keep the same files under an application directory.

Tool discovery follows deployment state rather than repository layout. Each service discovers decorated methods on its own framework-managed instances and exposes them through its authenticated MCP endpoint. A setup-selected discovery plugin watches trusted service records; the MCP client bridge calls `tools/list`, groups compatible replicas, and contributes descriptors and invocation clients through effects. Adding a method does not require a central setup row, but an agent must still name the logical tool in its allowlist before the model can request it.
...
2. The current `@tool` decorator marks methods on backend-managed application services for the installed MCP server plugin to expose.

These APIs do not form another Karaka definition format. Setup remains in one setup YAML, and agents and subagents remain ordinary Cordis plugin modules loaded by it. A backend's framework bootstrap and ordinary deployment configuration install its MCP server integration but do not define agents. Agent authors use normal plugin effects rather than a parallel `defineAgent` system.

A remote deployment uses role-specific plugins from the Tool family. Each backend bootstraps an application MCP server plugin that turns decorated metadata into Cordis-owned MCP handlers. Karaka's setup selects endpoint-discovery and MCP client plugins that turn verified MCP tool lists into Cordis-owned Agent Runtime descriptors and Tool-family clients. Each plugin is mounted once in its process and uses that process's service container, registries, effects, scopes, and dependency ordering. A shared-process deployment may mount both roles in one graph.
...
@tool metadata on a backend-managed method
        |
        v
application MCP server plugin
        |
        v
reversible MCP handler effects

trusted endpoint plus verified tools/list
        |
        v
...
The code API must not create parallel storage, authentication, Agent Runtime, lifecycle, tool, or plugin registries. In each process, the Chat API and MCP server plugins delegate to services and contributions in that process's Cordis graph; they do not own hidden registries or permanent global registrations. Two composition systems would duplicate dependency ordering, scoping, cleanup, and hot replacement, and would make Cordis lifecycle guarantees stop at the code API boundary.
...
- Mount one MCP server plugin from the Tool plugin family during each backend's bootstrap, enumerate its managed instances, bind decorated methods, and expose them through reversible Cordis effects.
- Discover trusted remote MCP endpoints through setup-selected static or service-discovery plugins, then consume schema-verified `tools/list` results and fail agent activation when required tools are missing or incompatible.
