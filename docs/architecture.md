...
On every turn, including after a process restart, Agent Runtime resolves the stored agent ID against the current active compiled agent and loads durable chat state into that behavior. The turn leases that immutable snapshot through every model generation and tool call. Replacing or reloading an agent, model, or tool plugin therefore changes subsequent turns while an already-running turn completes against its leased implementations. Tool calls and results use the versioned session format and survive process restart. Exact historical execution is a separate deployment policy that requires retaining an old executable artifact; a chat record cannot preserve code that is no longer deployed.
