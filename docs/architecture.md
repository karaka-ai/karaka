...
Every supported DSH Node application runs through the `dsh` profile launcher. The shipped applications are `dsh web` (the deliberate alias for `--profile web`), `dsh --profile headless`, `dsh --profile sdk`, `dsh --profile sdk-minimal`, and `dsh --profile acp`. The TypeScript SDK resolves its same-version `dsh` dependency and selects `sdk`; custom plugin composition remains a profile plus ordered patch files, not an inline application tree. `sdk-minimal` is a repository-owned standalone bundle behind the same launcher, not a caller-supplied Cordis tree.

Karaka has a separate publication boundary. `karaka start` resolves and launches `@karaka/agent/bin`; the Agent package owns its composition and embedded plugin registry and does not resolve or launch the `dsh` package.

Vendored CLIs, build-only and test-only executables, direct in-process plugin mounting, and the private browser WebWorker preview are not Harness application launchers. [`verify-application-entrypoints`](../scripts/verify-application-entrypoints.ts) keeps every package bin, executable source, product profile wrapper, and root demo in an explicit class and rejects a Node application path that bypasses `dsh`.
...
## Karaka application runtime

Karaka runs application-owned agents in the self-contained [`@karaka/agent`](../packages/karaka/agent/README.md) server. The application backend installs [`@karaka/sdk`](../packages/karaka/sdk/README.md); the separate Karaka process installs [`@karaka/cli`](../packages/karaka/cli/README.md), which brings the same-version Agent runtime, agent definitions, and any optional Cordis plugin packages. Installing the SDK does not start either process or open a port.

An agent definition is one Agent Preset directory. `preset.yml` supplies discovery metadata such as name, description, and order. `agent.cordis.yml` is the behavioral plugin composition: persona, selected remote tools, skills, subagents, and other scoped contributions. `@karaka/agent/*` names resolve from the embedded registry; relative paths resolve from the composition file; other bare names resolve from the Agent project's installed dependencies. Karaka mounts one standing Cordis plugin tree for each detected composition generation. Creating or resuming a chat gives its Agent scope the scoped contributions registered by its generation. Chats joined to the same generation share preset plugin instances but retain distinct Agent and Session state; a changed composition starts a new generation without disrupting chats still joined to the old one. A new chat starts with the deployment's default model selection; `chats.setModel()` takes effect on that chat's next request and remains its selection without changing the deployment default or other chats.

Karaka authenticates the application backend through `ctx.serverAuth`; the backend SDK sends its credential and trusted tenant and user identifiers through the HTTP/SSE transport. The transport binds `{ applicationId, tenantId, userId }` to the durable Session header before Session Controller creates or resumes the Agent. The same owner must match every later history, prompt, model, response, cancel, and stream operation.

Application tools travel in the opposite direction. The SDK mounts an authenticated MCP handler on the application's existing Node HTTP server. `@karaka/mcp-application` specializes the generic MCP client with outbound server authentication, Session identity forwarding, and Agent Preset tool selection. Every application-owned tool remains hidden until the selected preset explicitly names it in an `allow` list. The application verifies Karaka's credential before dispatching the registered callback.

`karaka start` runs the Loader, Agent Presets, ReactLoopAgent, model adapters, tool registry, Session log, SQLite persistence, authentication, and HTTP transport embedded in `@karaka/agent`. The default composition disables local filesystem and subprocess tools. One launch owns one Node process; replica count, routing between replicas, and TLS termination remain deployment responsibilities.
