...
There is no framework bootstrap code in your file: a plugin describes what it contributes, and `cordis.yml` composes the application. Karaka's [foundation example](../../examples/foundation/cordis.yml) uses the same mechanism for several cooperating plugins.
