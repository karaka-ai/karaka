...
Cordis is Karaka's plugin framework: a small runtime where capabilities are plugins mounted into a shared context. This tutorial teaches Cordis through runnable examples built in a scratch directory inside this repository.
...
If you want a condensed concept reference instead of a walkthrough, read the [Cordis primer](../cordis-primer.md). The [Cordis core API](../cordis-api/context.md) pages provide detailed lookup documentation.

For a complete repository example loaded from `cordis.yml`, see the [foundation example](../../examples/foundation/README.md).
...
```sh
git clone https://github.com/karaka-ai/karaka.git
cd karaka
pnpm install
```
...
- **`import type { Context } from '@karaka/cordis'`** imports only type information. It vanishes at runtime, so a plugin file that needs `Context` solely for annotations adds no runtime dependency.
- **Declaration merging** (`declare module '@karaka/cordis' { ... }`) adds your entries to interfaces that Cordis already declares — for example the type of a new `ctx.greeter` property or event name. It generates no runtime wiring; the plugin separately provides the service or emits the event. Chapter 3 shows the pattern in full.
