...
A **service** is a named capability one plugin provides and other plugins consume through `ctx`. A consumer names the capability rather than importing its provider, so configuration can select a provider without changing the consumer.
...
- **Compile time**: the `declare module '@karaka/cordis'` block is TypeScript declaration merging. It adds `greeter` to the `Context` interface so `ctx.greeter` typechecks everywhere. It generates no code; without it the service still works at runtime, but consumers lose type safety.
...
This is also why service replacement works in config: unload one provider, mount another provider for the same name, and every plugin injecting that service cleanly restarts against the new implementation.
...
Service names live in one flat namespace per application. Choose stable names that describe capabilities rather than providers.
