...
Groups nest a sub-list of entries that load and unload as one unit, and `isolate` gives a group its own instance of a service name. Two groups can therefore use differently configured providers for the same capability without affecting each other. The [Cordis primer](../cordis-primer.md) covers the details.

## Hot module replacement

Because unloading releases effects ([chapter 2](02-lifecycle-and-effects.md)) and loading follows dependencies ([chapter 3](03-services.md)), HMR can replace a running plugin by unloading and loading it. The `@karaka/cordis-plugin-hmr` plugin watches your files and does exactly that on save.

In `tmp/cordis-tutorial`, write `cordis.yml`:

```yaml
- id: logger
  name: '@karaka/cordis-plugin-logger-console'
- id: timer
  name: '@karaka/cordis-plugin-timer'
- id: hmr
  name: '@karaka/cordis-plugin-hmr'
  config:
    root: ['.']
- id: hello
  name: './hello.ts'
...
```ts
import { FiberState, type Context } from '@karaka/cordis'
...
`inject: ['timer']` has no provider. Add `- name: '@karaka/cordis-plugin-timer'` to the list and the plugin loads. When a plugin does nothing and reports nothing, inspect its fiber state. Iterating without the PENDING filter also shows the loader's own plugins (Loader, Include) as ACTIVE fibers because plugins mount the config file itself.

Next: apply the same pattern to a service definition, provider plugins, and consumers in your application.
