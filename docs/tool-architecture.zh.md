...
> 状态：工作架构。明确标记为未来工作的部分仍未解决；下文所述的核心注册表、MCP 桥接、激活验证、模型工具映射、工具循环和持久工具历史目前已经实现。
...
智能体插件声明它可以使用的逻辑工具 ID。发现使工具对运行时可用，但不会向所有智能体授予访问权。智能体激活会解析其模型和精确允许列表，并要求该模型提供方验证选中的描述符。并发 Loader 条目可能让智能体暂时保持等待状态，但如果任何必需能力仍然缺失或不兼容，整张图稳定后的就绪阶段会在普通启动返回前失败。

调用模型时，智能体运行时使用已编译的允许列表，并把每个选中工具的名称、描述和输入 schema 加入结构化、提供方中立的模型请求。工具系列不负责构造系统提示词。模型提供方插件把结构化定义转换成该提供方原生的函数调用格式。

模型返回结构化工具调用后，智能体运行时请求其租用的工具注册表视图执行调用。注册表验证参数、选择已经绑定的实现、验证其结果，并返回结构化 JSON。智能体运行时把调用、结果以及按顺序排列且 JSON 安全的提供方重放状态记录到带版本的持久对话中，将其加入下一次模型请求，然后继续运行，直到模型返回最终文本或达到该智能体的工具轮次上限。提供方重放状态对智能体运行时保持不透明。
...
Karaka 侧的所有注册都属于 Cordis effect。模型与工具注册表会暴露修订号，用于刷新激活结果。正在运行的一轮会租用稳定的智能体、模型与工具注册表视图；后续轮次看到当前已加载的插件。释放会阻止新调用，并等待现有租约完成。请求主体、工具调用和结果都是运行时数据，而不是按用户挂载的插件。
