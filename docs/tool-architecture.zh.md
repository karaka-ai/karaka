...
公开发布的 `@karaka/tool` 包在本仓库中实现，但由其他仓库中的应用后端使用。其包根提供惰性的方法装饰器，为方法附加稳定的工具 ID、描述、输入和输出 schema，以及必需的应用权限。导入经过装饰的服务不会注册全局行为，也不会启动服务器。
...
MCP 提供协议结构，但不会自动建立信任。远程边界必须使用可信发现和加密传输。Karaka 使用短期有效、绑定受众的凭据进行认证；应用在每次调用时验证凭据并解析被委托的主体。应用仍在本地执行装饰器声明的权限检查。

工具描述和结果都是不可信的模型输入。Karaka 在通信两端验证 schema，智能体只能使用允许列表中的逻辑工具，并且模型绝不选择端点 URL。除非操作具有明确的幂等契约，否则不会自动重试修改数据的调用。最终的认证与身份委托规范仍待确定。
