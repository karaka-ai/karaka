...
所有受支持的 Node 应用都通过 `dsh` profile launcher 运行。随附应用是 `dsh web`（刻意为 `--profile web` 保留的别名）、`dsh --profile headless`、`dsh --profile sdk`、`dsh --profile sdk-minimal` 与 `dsh --profile acp`。TypeScript SDK 会解析其同版本 `dsh` 依赖并选择 `sdk`；自定义插件组合继续由 profile 与有序 patch 文件表达，而不是内联应用树。产品 CLI 可以准备本地 profile 状态，并在委托给 `dsh` 前选择一个固定 profile；`karaka start` 就是 `karaka` profile 的这种受限包装层。`sdk-minimal` 是位于同一 launcher 后的仓库自有独立组合包，而不是由调用方提供的 Cordis 配置树。
...
Karaka 是用于应用自有 Agent 的持久 DSH profile。应用后端安装 [`@karaka/sdk`](../packages/karaka/sdk/README.zh.md)；独立 Karaka 进程安装 [`@karaka/cli`](../packages/karaka/cli/README.zh.md)、Agent 定义，以及这些定义引用的自定义 Cordis 插件。安装 SDK 不会启动任何进程或监听端口。

一个 Agent 定义就是一个 DSH Agent Preset 目录。`preset.yml` 提供名称、描述和顺序等发现元数据。`agent.cordis.yml` 是行为插件组合：persona、选中的远程工具、skill、subagent 和其他作用域贡献。Karaka 为每个检测到的组合世代挂载一棵常驻 Cordis 插件树。创建或恢复聊天时，其 Agent scope 获得所属世代注册的作用域贡献。加入同一世代的聊天共享 preset 插件实例，但各自保有独立的 Agent 与 Session 状态；组合发生变化时会启动新世代，而仍加入旧世代的聊天不会受到干扰。新聊天采用部署的默认模型选择；`chats.setModel()` 从该聊天的下一次请求起生效，并继续作为该聊天的选择，而不会更改部署默认值或其他聊天。
...
`karaka start` 复用 DSH launcher、Loader、Agent Presets、ReactLoopAgent、模型 adapter、工具 registry、Session log 与 SQLite 持久化。Karaka bundle 默认禁用本地文件系统和子进程工具。一次启动拥有一个 Node 进程；副本数、副本间路由和 TLS 终止仍由部署负责。
