...
Karaka 是基于 Cordis 的可配置基础层，用于组合智能体 SaaS 运行时。稳定的能力接缝定义运行时能做什么，提供方插件决定基础设施工作如何以及在何处完成，应用配置则选择实际运行的产品。设置中挂载的集成可以把普通应用函数转换为智能体工具，而无需开发者为每个函数编写一个插件。
...
基础设施能力应位于该基础层之上，并由第一方、第三方或私有插件提供。Karaka 可以发布契约和实用的提供方，但第一方提供方没有特权运行路径。应用自有函数可以使用 Karaka 的工具编写辅助 API，并继续作为普通应用代码。`vendor/` 包应与具体应用、提供方、部署目标或 SaaS SDK 保持独立。
...
2. 一份或多份**智能体 YAML**定义智能体和 subagent：提示词、逻辑模型与会话引用、允许的工具 ID、技能和委派关系。
...
flowchart TB
  ...
  Spec["一份逻辑部署规格"]
  AppLoader["应用进程 Loader"]
  AgentLoader["智能体进程 Loader"]
  AppGraph["应用进程的<br/>Cordis 图"]
  AgentGraph["智能体进程的<br/>Cordis 图"]
...
这两类 YAML 构成一份逻辑部署规格，并使用同一种组合模型：插件和由 effect 所有的贡献项。每个进程拥有自己的 Cordis context 和图。共享进程部署只有一张图；分布式部署则从同一规格中加载相关角色，分别构成应用、智能体或 worker 图。

下面是说明性的部分设置片段，它选择一些提供方并指向智能体文件。提供方名称和配置结构都属于规划中的 API，并非当前 API：
...
每个智能体文件包含智能体或 subagent 定义，而不是插件 Loader 配置项：
...
规划中的加载层将解析这两类 YAML，并通过 Cordis 挂载其运行时表示。智能体定义将在内部成为由 effect 所有的智能体运行时注册项；开发者无需编写智能体插件。直接使用 Cordis 组合仍适用于 Karaka 内部、测试、嵌入式集成和高级插件作者，但不是第三种普通配置界面。
...
每个部署在各接缝内组合所需插件。Karaka 可以发布默认契约和提供方，应用也可以用普通 Cordis 插件添加或替换它们。自定义插件仍是高级扩展路径。普通应用函数使用下文的工具辅助 API，无需开发者编写插件。
...
同一模式也适用于 SaaS 领域，但普通应用开发者不应为每个函数编写插件或设置项。Karaka 将提供装饰器及等价的函数辅助 API。下面的 API 仅用于说明：
...
`@tool` 和 `defineTool` 只附加元数据。导入装饰过的函数不会注册函数，也不会修改全局注册表。设置 YAML 将挂载一次应用工具宿主集成。应用启动时，该集成将枚举由框架或应用管理的实例，读取其工具元数据，绑定对应方法，再通过可逆 Cordis effect 注册本地执行处理器。宿主插件拥有这些 effect，因此释放插件时会移除处理器。辅助 API 不得创建第二套服务容器、生命周期、注册表或不可释放的全局旁路。

在远程部署中，应用进程的工具宿主插件将通过经过认证的通道提供带版本的已绑定工具 manifest。智能体进程的桥接插件将认证宿主、获取并验证 manifest，再通过自身的可逆 effect 向智能体运行时注册模型可见描述符。因此，应用图拥有处理器 effect，智能体图拥有描述符 effect。共享进程的开发或嵌入式部署可以把两个角色组合在一张图中，而不改变各自的所有权。

工具宿主将为所有装饰过的函数暴露一个 manifest 操作和一个调用分发器；装饰器不会为每个函数创建一条网络路由。工具 RPC 认证分为两层。mTLS 或服务凭据等服务认证证明调用来自获准的 Karaka 部署；短期签名委派则携带本次调用可以使用其权限的已验证主体和租户。模型不能提供或修改这两种身份。每次调用时，应用宿主都会认证服务和委派、验证输入、通过授权接缝执行工具声明的权限、调用已绑定函数、验证输出并记录审计事件。

概念上的调用信封包含调用 ID、逻辑工具 ID 与版本、已验证输入、截止时间和签名委派。凭据与实际主体属于传输元数据，绝不是模型可见的工具参数。Manifest 与调用协议是共享契约，认证机制则仍是由设置选择的可替换提供方。

设置 YAML 将对应用目标及其传输只配置一次，而不是为每个函数分别配置。远程执行很可能是生产应用自有工具的默认选择。本地执行将主要用于开发、测试和嵌入式部署。两种位置都会向智能体运行时暴露相同的逻辑工具 ID。
...
注册工具只表示运行时可以找到它，并不授予每个智能体调用权限。如果允许列表中的工具不在已验证 manifest 中，或其版本或 schema 不兼容，智能体激活必须失败。智能体运行时会先验证模型参数，再要求执行接缝传输调用。应用宿主会再次验证输入，使用内部传递的主体授权请求，执行绑定方法，并验证输出。智能体运行时还会验证返回的输出，再将其交给模型。高级开发者可以通过自定义插件直接贡献工具，但这些贡献必须使用同一注册表和 effect 生命周期。
...
聊天 ID 只是定位符，绝不是权限凭证。Karaka 必须认证并授权每次操作，防止用户仅凭另一个用户的聊天 ID 获得访问权。一个 Karaka 实例和一份智能体定义可以服务多个并发调用方；每个聊天保有独立的所有权和状态，无需挂载逐请求 Cordis 插件。
...
开发者将在智能体 YAML 中定义所有智能体和 subagent，Karaka 则会创建并运行它们。智能体定义是声明式数据，不是普通应用代码需要组装的活对象、TypeScript 回调或插件。subagent 是另一份由父智能体按名称引用的智能体定义。
...
智能体定义中的引用将指向逻辑能力或策略，而不是具体提供方对象或端点。装饰过的应用函数和高级工具插件将提供逻辑工具名称；设置中选择的模型与会话提供方将满足其他引用。因此，将 OpenAI 替换为 DeepSeek、将 PostgreSQL 替换为另一种会话后端，或者替换远程执行传输，都不需要重写智能体。

智能体运行时集成将读取设置中指定的智能体文件，并将其转换为可逆的 Cordis 注册项。文件移除、替换和热重载因此将使用与插件贡献项相同的注册表生命周期。这种内部转换将保留 Cordis 分层，同时不要求每个智能体对应一个开发者编写的插件或设置项。
...
智能体定义、扩展、路由器、工具语义、模型策略、会话策略和委派语义都位于智能体运行时接缝内部。它们可以使用 `ctx.agentRuntime` 等内部注册表，但不会因此成为新的顶层接缝。高级插件可以扩展或生成定义，但普通开发者界面仍是智能体 YAML。
...
一个进程会把多个智能体定义加载到同一个智能体运行时注册表中。智能体是具名定义，不是进程；因此，新增支持、计费、研究或报告智能体不需要再启动一台服务器。每个聊天解析一份定义，并携带独立的主体、会话、历史和执行状态。subagent 是另一份定义，最初通过同一运行时执行，除非执行策略把它放入隔离或远程 worker。

```mermaid
flowchart LR
  Frontend["前端"] --> Application["应用后端<br/>公共 API 与装饰过的函数"]
  Application -->|"聊天请求"| Karaka["持久 Karaka 服务器<br/>聊天 API 与智能体运行时"]
  Karaka -->|"经过认证的工具 RPC"| ToolHost["应用工具宿主"]
  Karaka --> Support["支持智能体定义"]
  Karaka --> Billing["计费智能体定义"]
  Karaka --> Research["研究智能体定义"]
```

因此，基础生产拓扑由现有应用后端和一台 Karaka 服务器组成。应用部署包含装饰过的函数与工具宿主插件；Karaka 部署包含智能体运行时、智能体定义、模型提供方和远程执行桥接插件。两个部署都不会吸收对方的实现。

容量扩展通过部署平台的负载均衡器运行多个完全相同的 Karaka 副本。每个副本加载相同版本的设置和智能体定义。聊天所有权、历史、会话状态与执行元数据必须存放在共享的持久存储中，而不是进程内存中，使任何副本都能继续聊天。副本数量属于 Docker、Kubernetes、ECS、systemd 或其他部署系统；设置可以定义运行时并发与资源策略，但智能体 YAML 绝不定义进程数量。

只有出现具体的位置需求时才引入额外 worker 角色，例如不可信沙箱执行、持久后台任务、隔离 subagent 或自托管推理。执行接缝负责路由这些已解析的工作负载，无需修改智能体定义，也不会为每个智能体创建一个进程。
...
2. `@tool` 和 `defineTool` 等工具辅助 API 标记普通应用函数，交由设置中安装的应用工具宿主进行注册。

这些 API 不构成另一种配置界面。设置仍位于一份设置 YAML 中，智能体和 subagent 仍位于智能体 YAML 文件中。尤其是，普通 API 不提供与智能体 YAML 平行的编程式 `defineAgent` 路径。

远程部署的设置会选择两个面向不同角色的集成插件。应用工具宿主把装饰器元数据转换为由 Cordis 所有的执行处理器；智能体桥接插件把经过验证的 manifest 转换为由 Cordis 所有的智能体运行时描述符。每个插件都在各自进程中挂载一次，并使用该进程现有的服务容器、注册表、effect、作用域和依赖顺序。共享进程部署可以在一张图中同时挂载这两个角色。
...
高级开发者可以编写普通 Cordis 插件，以添加或替换服务、提供方、策略、注册表和生成的贡献项。这些插件需要模块入口和可序列化的设置配置，Loader 才能部署它们。直接编程式组合仅保留给内部代码、测试、需要运行时值的嵌入式集成和自定义插件作者。
...
Cordis 服务隔离应用于插件图组合，例如静态配置的子树需要不同实现或注册表视图时。不要使用服务隔离来表达每个请求、主体、消息或聊天；它们是运行时数据，逐调用权限必须在内部沿执行路径传递。作用域会收窄服务解析范围，但不会自动建立或复制权限。策略应附加到其治理的服务或执行接缝上，使包括面向模型的工具在内的每个消费者都经过相同的强制执行点。
...
- 只保留一套组合系统：Cordis。
- 使用一份逻辑部署规格，并让每个进程拥有一张 Cordis 图。
- 只暴露两类普通配置界面：一份设置 YAML 和一份或多份智能体 YAML。
- 将运行时组装、提供方、传输、策略和智能体文件位置放入设置 YAML。
- 将智能体、subagent、提示词、逻辑能力引用和工具允许列表放入智能体 YAML。
...
- 将智能体 YAML 转换为引用逻辑能力和策略的、可逆的智能体运行时贡献项。
- 不要求普通开发者编写智能体插件或使用编程式智能体定义 API。
- 将主体、聊天、消息、响应和调用保留为运行时数据，而不是 Cordis 插件或服务。
- 通过范围狭窄的工具暴露模型操作；不要隐式暴露整个服务。
- 让普通应用函数通过 `@tool` 或 `defineTool` 成为工具，无需逐函数 YAML 或开发者编写的插件。
- 装饰器只附加元数据；导入应用代码不得修改注册表。
- 从设置中只挂载一次工具宿主集成，枚举受管理实例，绑定装饰过的方法，并通过可逆 Cordis effect 注册它们。
- 通过经过认证、带版本且完成 schema 验证的 manifest 发现远程工具；缺少必需工具或工具不兼容时，智能体激活必须失败。
- 将工具语义和允许列表留在智能体运行时，将位置、传输和调用放入执行接缝。
- 对应用执行目标只配置一次；生产应用工具优先远程执行，本地执行主要用于开发或嵌入式场景。
- 通过一个经过认证的 manifest 和调用分发器暴露装饰过的应用函数，而不是为每个函数创建一条路由或设置项。
- 每次远程工具调用都要认证 Karaka 服务与短期委派主体；绝不从模型参数中接受权限。
- 将工具注册与其消费的应用服务或函数分开。
- 在智能体运行时中解析 subagent 继承规则和完整子调用；执行接缝只负责放置和传输。
- 将智能体和 subagent 视为共享运行时注册表中的定义，而不是进程。
...
- 不要让产品行为进入九包内核；普通应用代码承载装饰过的函数，自定义插件承载高级扩展。
