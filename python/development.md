...
When `DEEPSEEK_API_KEY_EXTERNAL` is configured, trusted pull requests also run `--scenario sdk-live --installed-wheel` on every native target. That scenario performs two tool-using turns against `https://api.deepseek.com` and verifies the created file externally. Runs without the secret, including fork and Dependabot pull requests, skip the live scenario but still run the complete keyless installed-wheel path.
