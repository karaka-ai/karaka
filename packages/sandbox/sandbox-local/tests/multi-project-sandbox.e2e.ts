...
async function projectDir(label: string): Promise<string> {
  // Platform temp areas are writable by policy, so isolation fixtures belong outside them.
  const dir = await mkdtemp(join(dirname(fileURLToPath(import.meta.url)), `.dsh-${label}-`))
  tempDirs.push(dir)
  return dir
}
...
beforeEach(async () => {
  ...
  await ctx.plugin(ShellEnv, { dshHome: fallbackRoot })
