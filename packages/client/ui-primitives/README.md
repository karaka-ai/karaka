...
Supported `CodeBlock` and `ReadBlock` content stays plain until its first viewport intersection, then retains highlighting for the component lifetime. Unknown languages stay plain. Lazy grammars load when an activated block first needs them; source text and copy behavior remain unchanged ([viewport activation](../../../.agents/notes/implemented/architecture/2026-08-31-viewport-activated-syntax-highlighting.md)).
