...
describe('continuable policy inheritance', () => {
  it('keeps application ownership, delegation policy and lineage across a host restart', async () => {
    ...
    const initial = await loadStoredSession(first.ctx.sessionPersistence, started.childId)
    ...
    const resumed = await loadStoredSession(second.ctx.sessionPersistence, started.childId)
