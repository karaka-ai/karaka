...
if (typeof graph.sourceRevision !== 'string' || !/^[a-f0-9]{40}$/.test(graph.sourceRevision)
  || typeof graph.sourceDirty !== 'boolean') throw new Error('Runtime artifact requires checkout revision metadata; rebuild it with the current materializer')
...
await writeFile(resolve(output, 'BUNDLED-PACKAGES.json'), JSON.stringify({ sourceRevision: graph.sourceRevision, sourceDirty: graph.sourceDirty, runtime: 'linux-x64-glibc', packages: placements }, null, 2) + '\n')
await writeFile(resolve(output, 'README.md'), `# Karaka Agent\n\nApplication server assembled from Karaka checkout ${graph.sourceRevision}${graph.sourceDirty ? ' with local changes' : ''}, using DeepSeek Harness and Karaka identity and transport plugins.\n\nThis prerelease supports Linux x64 with glibc 2.28 or newer and Node.js ^22.19.0 or >=24.0.0. Other platforms are not included. Install the matching @karaka-ai/cli prerelease to scaffold and launch a project.\n\nLocal tool plugins import defineTool, ToolArgsError and authoring types from @karaka-ai/agent/tools; these reexport the original DSH tool implementation.\n\nThe browser client is exported from @karaka-ai/agent/browser. Browser deployments must configure JWT verification and allowed origins.\n\nThe package bundles its exact runtime dependencies; no install hooks or upstream package publication are required. Shared upstream extensions and Karaka adapters are maintained in the source checkout. See BUNDLED-PACKAGES.json and each dependency's license notices. Bundled dependencies retain their own licenses.\n\nAuthoritative chats use upstream JSONL with a separate Karaka ownership store; back up both. No historical SQLite migration is included.\n`)
