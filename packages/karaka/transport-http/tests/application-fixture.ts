...
import { ApplicationId, TenantId, UserId, sameOwner, IdentityError, type ApplicationOwner } from '@karaka-ai/identity'
...
export async function applicationFixture(mountController = true): Promise<ApplicationFixture> {
  ...
  ctx.provide('karakaIdentity', identity as unknown as typeof ctx.karakaIdentity)
  const presets = {
  ...
  const removePresets = ctx.provide('agentPresets', presets as unknown as typeof ctx.agentPresets)
  ...
  ctx.provide('llm', llm as unknown as typeof ctx.llm)
  const observed = vi.fn()
  const observeSession = vi.fn(async (id: SessionId) => ({ events: [...(events.get(id) ?? [])], [Symbol.dispose]: observed }))
  ctx.provide('sessionQuery', { observeSession } as unknown as typeof ctx.sessionQuery)
  async function make(id: SessionId, setup: AgentSetup | undefined, options: AgentOptions = {}, header?: SessionHeader) {
    ...
    const cancel = vi.fn()
    const agent = { id, session, ctx: ctx.extend(), options, followup, cancel } as unknown as Agent
