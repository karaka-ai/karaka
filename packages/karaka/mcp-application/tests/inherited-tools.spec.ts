/** DSH MCP tool regressions adapted to the application bridge; derivation is recorded in UPSTREAM.json. */
...
import { publicToolName, syncTools as syncApplicationTools, type ToolBridgeOptions } from '../src/tools.ts'
...
const defaultOpts: Omit<ToolBridgeOptions, 'bridge'> = {
  registrationFailure: 'contain',
  serverName: 'srv',
  toolCallTimeoutMs: 60_000,
}

/** Preserve inherited tool assertions while supplying Karaka's registration and metadata hooks. */
function syncTools(
  client: Client,
  ctx: Context,
  opts: Omit<ToolBridgeOptions, 'bridge'>,
  previous: Map<string, () => void>,
) {
  return syncApplicationTools(client, ctx, {
    ...opts,
    bridge: {
      headers: async () => ({}),
      metadata: async () => ({}),
...
describe('syncTools', () => {
  ...
  it('propagates strict registration failure after rolling back the entire new catalog', async () => {
    ctx.tools.register({
      name: 'mcp__srv__taken', description: 'Existing owner', parameters: { type: 'object' },
      output: { schema: { type: 'string' }, render: (_args, value) => [{ type: 'text', text: value as string }] },
      execute: async () => 'existing',
    })
    const original = ctx.tools.get('mcp__srv__taken')
    const client = createMockClient([
      { name: 'free', inputSchema: { type: 'object' } },
      { name: 'taken', inputSchema: { type: 'object' } },
    ])
    await expect(syncTools(client as never, ctx, { ...defaultOpts, registrationFailure: 'throw' }, new Map()))
      .rejects.toThrow()
    expect(ctx.tools.get('mcp__srv__free')).toBeUndefined()
    expect(ctx.tools.get('mcp__srv__taken')).toBe(original)
  })
...
describe('tool execution', () => {
  ...
  it('calls MCP callTool with the RAW name and returns text content', async () => {
    ...
    expect(client.callTool).toHaveBeenCalledWith(
      { name: 'echo', arguments: { msg: 'hi' }, _meta: {} },
      undefined,
  ...
  it('sends the raw name for normalized public names', async () => {
    ...
    expect(result.isError).toBe(false)
    expect(client.callTool).toHaveBeenCalledWith(
      { name: 'admin.reset', arguments: {}, _meta: {} },
      undefined,
      expect.anything(),
    )
  })
