/**
 * DSH connection-supervisor regressions adapted to Karaka application hooks: crash-driven reconnection
...
import { RECONNECT_DEFAULTS, resolveReconnectPolicy, startConnection as startApplicationConnection, type ResolvedReconnectPolicy } from '../src/connection.ts'
...
async function mountRegistry(): Promise<Context> {
  ...
  onTestFinished(async () => {
    for (const instance of instances) instance.onclose?.()
...
function connectionConfig(reconnect?: Config['reconnect']): Config {
  return {
    applicationId: 'app',
    serverName: 'srv',
...
/** Supply application hooks while exercising the inherited supervisor in isolation. */
function startConnection(ctx: Context, config: Config, policy: ResolvedReconnectPolicy) {
  return startApplicationConnection(ctx, config, policy, {
    headers: async () => ({}),
    metadata: async () => ({}),
...
async function apply(ctx: Context, config: Config): Promise<void> {
  const connection = startConnection(ctx, config, resolveReconnectPolicy(config.reconnect, 'reconnect'))
...
describe('reconnect supervisor', () => {
  ...
  it('reconnects after a transport close, re-syncs tools through the new generation, and serves calls', async () => {
    ...
    await apply(ctx, connectionConfig({ initialDelayMs: 5, maxDelayMs: 40, maxAttempts: 5 }))
  ...
  it('stops at the failure cap, unregisters the tools, and reports final failure', async () => {
    ...
    await apply(ctx, connectionConfig({ initialDelayMs: 2, maxDelayMs: 8, maxAttempts: 2 }))
    await vi.waitFor(() => { expect(ctx.tools.get('mcp__srv__remote')).toBeDefined() })
  ...
  it('gives up behind an in-flight re-sync and removes the generation it publishes', async () => {
    ...
    await apply(ctx, connectionConfig({ initialDelayMs: 2, maxDelayMs: 8, maxAttempts: 1 }))
    await vi.waitFor(() => { expect(ctx.tools.get('mcp__srv__remote')).toBeDefined() })
  ...
  it('does not start a replacement until a failed generation reports that it closed', async () => {
    ...
    // harness's second close call returns, but the connection has not closed yet.
    mockClose.mockResolvedValue(undefined)

    const applying = apply(ctx, connectionConfig({ initialDelayMs: 2, maxDelayMs: 8, maxAttempts: 2 }))
  ...
  it('suppresses retry reporting when disposal owns a pending connect rejection', async () => {
    ...
    const handle = startConnection(ctx, connectionConfig(), resolveReconnectPolicy(undefined, 'reconnect'))
  ...
  it('dispose during the backoff wait cancels the pending reconnect', async () => {
    await apply(ctx, connectionConfig({ initialDelayMs: 60_000, maxDelayMs: 60_000, maxAttempts: 5 }))
    await vi.waitFor(() => { expect(ctx.tools.get('mcp__srv__remote')).toBeDefined() })
  ...
  it('a transport close after dispose schedules nothing', async () => {
    const fiber = ctx.plugin({ name: 'mcp-client', inject: ['tools'], apply }, connectionConfig())
    await vi.waitFor(() => { expect(ctx.tools.get('mcp__srv__remote')).toBeDefined() })
  ...
  it('reconnect disabled keeps the registered tools and reports manual recovery', async () => {
    const { errors } = captureLogs(ctx)
    await apply(ctx, connectionConfig({ enabled: false }))
    await vi.waitFor(() => { expect(ctx.tools.get('mcp__srv__remote')).toBeDefined() })
  ...
  it('an uptime past the stability window resets the attempt budget', async () => {
    const { errors } = captureLogs(ctx)
    await apply(ctx, connectionConfig({ initialDelayMs: 2, maxDelayMs: 30, maxAttempts: 1 }))
    await vi.waitFor(() => { expect(ctx.tools.get('mcp__srv__remote')).toBeDefined() })
  ...
  it('a crash loop with briefly successful connects still exhausts the cap', async () => {
    const { errors } = captureLogs(ctx)
    await apply(ctx, connectionConfig({ initialDelayMs: 2, maxDelayMs: 10_000, maxAttempts: 1 }))
    await vi.waitFor(() => { expect(ctx.tools.get('mcp__srv__remote')).toBeDefined() })
  ...
  it('a connect rejection racing its own transport close schedules exactly one retry per attempt', async () => {
    const { errors } = captureLogs(ctx)
    await apply(ctx, connectionConfig({ initialDelayMs: 2, maxDelayMs: 8, maxAttempts: 3 }))
    await vi.waitFor(() => { expect(ctx.tools.get('mcp__srv__remote')).toBeDefined() })

    // Each reconnect attempt sees the HTTP transport die (onclose) AND its
    // connect() reject — the real SDK emits both for a connection failure.
  ...
  it('dispose during an in-flight initial sync quiesces without leaking tools', async () => {
    const fiber = ctx.plugin({ name: 'mcp-client', inject: ['tools'], apply }, connectionConfig({ initialDelayMs: 2, maxDelayMs: 8, maxAttempts: 5 }))
    await vi.waitFor(() => { expect(ctx.tools.get('mcp__srv__remote')).toBeDefined() })
  ...
  it('reports a live catalog refresh failure and retains its previous tools', async () => {
    const { errors } = captureLogs(ctx)
    await apply(ctx, connectionConfig())
  ...
  it('a stale notification handler from a replaced generation is ignored', async () => {
    await apply(ctx, connectionConfig({ initialDelayMs: 2, maxDelayMs: 8, maxAttempts: 5 }))
    await vi.waitFor(() => { expect(ctx.tools.get('mcp__srv__remote')).toBeDefined() })
...
describe('resolveReconnectPolicy', () => {
  ...
  it('apply fails loud at load on a misconfigured reconnect', async () => {
    ...
    await expect(apply(ctx, connectionConfig({ initialDelayMs: 100, maxDelayMs: 5 })))
