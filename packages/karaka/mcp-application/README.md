...
The [provenance record](UPSTREAM.json) identifies connection and result-conversion code derived from the pinned [DSH MCP client](../../mcp/mcp-client/README.md). Application-specific hooks supply credentials, verified invocation metadata and scoped registration. The upstream public plugin does not expose these hooks.
