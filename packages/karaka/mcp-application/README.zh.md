...
本包通过共享的 [DSH MCP 客户端扩展](../../mcp/mcp-client/README.zh.md#programmatic-extensions)复用连接监督、发现和丰富结果转换。Karaka 与 DSH 使用相同的 MCP SDK 客户端，并提供经过认证的 HTTP 传输、经过验证的调用元数据、严格的模式准入和作用域目录注册。公开工厂在分派前将原始工具执行对象传给元数据钩子；结果转换与终结仍由上游实现负责。[上游来源记录](UPSTREAM.json) 标识共享实现与应用自有适配器。
...
- 支持应用 MCP 工具；此桥接器不发布应用资源、服务器指令、提示或基于任务的执行。不受支持的输入模式拒绝接收，不受支持的输出模式退回为 JSON 值。
