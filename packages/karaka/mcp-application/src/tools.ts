...
import type { ApplicationBridge } from './index.ts'
...
async function callToolUncached(
  ...
  return client.request(
    { method: 'tools/call', params: { name: rawName, arguments: args, _meta: await opts.bridge.metadata(exec) } },
...
export async function syncTools(
  ...
  try {
    for (const [publicName, definition] of definitions) {
      disposers.set(publicName, opts.bridge.register(definition))
    }
  } catch (error) {
