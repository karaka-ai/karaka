...
`@karaka-ai/cli` 创建 Agent 项目并启动其已安装的 `@karaka-ai/agent` runtime。`karaka init` 写入 Cordis 部署 patch、一个 Agent Preset、`plugins/` 目录以及 CLI 与 Agent 包的匹配直接依赖，且不覆盖已有文件。`karaka start` 使用项目私有的 `.karaka` home 在前台运行 Agent。
...
- `karaka start --config <path>` 准备项目本地 `.karaka` home 并启动 `@karaka-ai/agent/bin`；默认 patch 为 `karaka.cordis.yml`。
