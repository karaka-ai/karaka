...
- `websocketHeartbeatIntervalMs` 设置 Ping 周期。两个间隔内未收到回复且延后的最终检查仍未通过时，Host 会终止连接；事件循环或网络停顿更久的部署必须调大此间隔。
