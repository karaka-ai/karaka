...
if (typeof config.initializeStartDates !== 'boolean') {
  throw new Error('config.initializeStartDates 必须为 boolean')
}

function validateStartDateConfig() {
...
async function projectContext(number, includeStatusActor = false, includeStartDate = false) {
  const data = await graphql(
    ...
    }`,
    {
      ...
      includeStatusActor,
      includeStartDate,
      startDateField: includeStartDate ? config.startDateField : '',
    },
  )
  ...
  if (startDateField?.isIssueField) {
    throw new Error(`Project ${config.startDateField} 必须为 Project Date 字段，不得为 Issue 字段`)
  }
...
/**
 * Initialize one Issue's empty Project Date field when date initialization is enabled.
...
export async function initializeIssueStartDate(number, date) {
  if (!config.initializeStartDates) return
  validateStartDateConfig()
...
/**
 * Initialize referenced Issues from an opened PR when configured; disabled mode reads no dates.
...
export async function initializePullRequestStartDates(
  ...
  if (!config.initializeStartDates || action !== 'opened') return
  validateStartDateConfig()
...
/**
 * Apply Issue lifecycle events using the configured repository and optional date initialization.
 * @param {string} eventName GitHub event name.
 * @param {object} event GitHub event payload.
 * @returns {Promise<void>} Resolves after lifecycle updates and policy auditing.
 */
